  ...
  const fetchCourse = async () => {
    try {
      const res = await fetch(
        ...
        }/api/user/modules-with-lectures/${courseId}?search=${encodeURIComponent(
  ...
  const handleEnrollCourse = async () => {
    ...
    try {
      ...
      if (data?.courseId) {
        ...
        setTimeout(() => {
          router.push(`/dashboard/${userId}/enrolled-courses/${data.courseId}`);
        }, 1000);
      } else {
        setToastMessage({
          type: "Error",
    ...
    <div>
      ...
      {(enrolledCourses as { course: string }[]).some(
        ...
        <div className="mt-[2rem]">
          ...
          <Link
            href={`/dashboard/${userId}/enrolled-courses/${courseId}`}
      ...
      {toastMessage.type === "Error" && (
        <p className=" text-sm text-red-500 mt-2 ">{toastMessage.message}</p>
      )}
      ...
      <div className=" mt-[2rem] ">
        ...
        <p className=" md:text-lg text-[#2e2e2e] mt-[.5rem] ">
          {course?.description}
        </p>
      </div>
...
const InstructorInfo = () => {
  return (
    <div
      className={` w-full lg:max-w-[25rem] h-fit border rounded-lg p-[1rem] transition-all duration-300 ease-in-out `}
    >
